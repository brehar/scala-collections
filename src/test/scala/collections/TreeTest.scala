...
class TreeTest extends TestStyle {
  ...
  test("remove on a nonempty Tree should yield a new Tree without the element") {
  ...
  test("remove removes only the element in question") {
    ...
    val treeWithElements = Tree(first, second)
    treeWithElements.contains(first) shouldBe true
    treeWithElements.contains(second) shouldBe true

    val treeWithoutElement = treeWithElements.remove(first)
  ...
  test("remove removes only the element in question (ordering test)") {
    ...
    val treeWithoutElement = treeWithElements.remove(second)
  ...
  private def testRendering(set: Tree[_])(expected: String): Unit = {
